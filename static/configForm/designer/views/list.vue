  ...
  <div class="layout-container">
    ...
    <el-tabs v-model="activeTab">
      ...
      <el-tab-pane label="本地暂存的配置化列表" name="customer">
        ...
        <m-table :data="tableData2" :page-size="99999" :default-columns="['index']">
          ...
          <el-table-column width="300px" label="操作">
            <template slot-scope="scope">
              <yl-space :size="[8, 4]" wrap>
                ...
                <a @click="goPreview(scope.row, 'local')">预览</a>
                <a @click="goEdit(scope.row, scope.row.type)">编辑</a>
                <a @click="removeLocal(scope.row.name)">删除</a>
              </yl-space>
            </template>
          </el-table-column>
        </m-table>
...
export default {
  ...
  methods: {
    ...
    async getCustomerMetadataList() {
      const data = localStorage.getItem('customer_metadata_list');
      if (data) {
        try {
      ...
      this.$confirm(tip, '提示', {
        ...
        .then(async () => {
          ...
          if (res.errcode === 0) {
            ...
            this.$errorTips(res.errmsg || '启用失败');
          }
        })
        .catch(() => {});
    },
      ...
      this.$confirm(tip, '提示', {
        ...
        .then(async () => {
          ...
          if (res.errcode === 0) {
            ...
            this.$errorTips(res.errmsg || '同步失败');
          }
        })
        .catch(() => {});
    },
