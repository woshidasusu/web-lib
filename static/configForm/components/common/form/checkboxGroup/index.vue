<template>
  ...
  >
    <m-checkbox-group
      ...
      :cancel-confirm="!!+formTemplate.cancelConfirm"
      v-on="onEvents"
    >
      <el-checkbox
        v-for="(item, i) in options"
        :key="i"
        :label="item[fieldConfig.value] || item[fieldConfig.key]"
...
<script>
import { MCheckboxGroup } from '@/components/mui';
const debug = window.__debug_log__;
const log = (...args) => {
  if (debug) {
    console.log('[checkboxGroup]：', ...args);
  }
};
export default {
  components: {
  ...
  methods: {
    ...
    validateMetadata() {
      ...
      if (!dataSource || typeof dataSource !== 'object') {
        console.error('checkboxGroup 表单组件的 dataSource 字段不能为空');
        return false;
      }
      return true;
    },
